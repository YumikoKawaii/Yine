...
func (r Role) ChangeRole(conv_id string, user string, role string) {
	db.Exec("update conversations set role = ? where conv_id = ? and user = ?", role, conv_id, user)
}

func (r Role) RemoveUser(conv_id string, user string) {
	db.Exec("delete from conversations where conv_id = ? and user = ?", conv_id, user)
}
...
func (r Role) ChangeNickname(conv_id string, user string, nickname string) {
	...
	db.Exec("update conversations set nickname = ? where conv_id = ? and user = ?", nickname, conv_id, user)
...
func (r Role) IsMember(conv_id string, user string) bool {
	...
	result := ""
	db.Raw("select role from conversations where conv_id = ? and user = ?", conv_id, user).Scan(&result)
	return result != ""
...
func (r Role) RemoveConversation(conv_id string) {
	...
	db.Exec("delete from conversations where conv_id = ?", conv_id)
...
func (r Role) IsConversationExist(conv_id string) bool {
	...
	result := ""
	db.Raw("select distinct conv_id from conversations where conv_id = ?", conv_id).Scan(&result)
	return result != ""
}
...
func (r Role) IsConversationBetween(user string, guest string) bool {
	...
	result := ""
	db.Raw("select user from conversations where user = ? and conv_id = ?", user, guest).Scan(&result)
	return result != ""
}
...
func (r Role) GetReceivers(conv_id string, id string) []string {
	...
	db.Raw("select user from conversations where conv_id = ? and user <> ?", conv_id, id).Scan(&result)
	return result
...
func (r Role) GetAllConversation(id string) []string {
	...
	result := make([]string, 0)
	db.Raw("select conv_id from conversations where user = ?", id).Scan(&result)
	return result
}
...
func (r Role) GetGroupMember(gid string) []Role {
	...
	result := make([]Role, 0)
	db.Raw("select * from conversations where conv_id = ?", gid).Scan(&result)
	return result
...
func (r Role) GetPartner(user string, conv_id string) Role {
	...
	result := Role{}
	db.Raw("select * from conversations where conv_id = ? and user = ?", user, conv_id).Scan(&result)
	return result
